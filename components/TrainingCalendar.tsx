  ...
  return (
    <div className='space-y-6'>
      {weeks.map((week) => (
        <div
          key={week.weekStart}
          className='bg-zinc-900/60 border border-zinc-800 rounded-2xl p-4 md:p-6'
        >
          {/* Week header */}
          ...
          <div className='hidden md:block'>
            ...
            <div className='grid grid-cols-7 gap-2 mb-2'>
              {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => (
                <div
                  key={day}
                  className='text-center font-mono text-xs text-zinc-600 uppercase tracking-wider'
                >
                  {day}
                </div>
              ))}
          ...
          <div className='md:hidden space-y-2'>
            {week.days.map((day) => {
              ...
              if (isRest) {
                bgClass = 'bg-zinc-950 border-zinc-800/50';
              } else if (isRace) {
              ...
              return (
                <div
                  key={day.date}
                  className={`border rounded-lg p-3 flex items-start gap-3 ${bgClass}`}
                >
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    {day.target_distance_m && (
                      <div className='text-xs text-zinc-500 mt-1'>
                        {(day.target_distance_m / 1000).toFixed(1)}km
                      </div>
                    )}
                  </div>
                  {statusDot}
